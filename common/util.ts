...
export const humaniseDate = (date: string | Date): string =>
  stringToDate(date).toLocaleString('en-GB', {
    day: 'numeric',
    month: 'short',
    year: 'numeric'
  })
...
export const dateString = (date: Date | string = new Date()): string =>
  formatISO(stringToDate(date), { representation: 'date' })
...
export const relativeDate = (date: Date | string): string => {
  ...
  return split.length > 1 ? split[0] : humaniseDate(date)
}
