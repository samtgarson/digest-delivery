...
import { Frequency } from 'src/lib/util/frequency'
  ...
  return (
    <>
      <p className='flex items-center mb-3 sm:mb-0'>
        <Icon className='inline-block mr-2' /> Your digest is{' '}
      ...
      <Btn
        ...
        className='sm:w-60'
        secondary={active}
        onClick={() => updateUser({ active: !active })}
      >
...
const FrequencyToggle: FC<UserFormProps> = ({
  updateUser,
  user: { frequency }
}) => (
  <>
    ...
    <RadioList
      value={frequency}
      items={items}
      update={frequency => updateUser({ frequency })}
      className='sm:text-right flex-grow'
    />
  </>
)

const AddressField: FC<UserFormProps> = ({
  updateUser,
  user: { kindleAddress: address }
}) => (
  <>
    <label htmlFor='kindle-address-input' className='mb-3 sm:mb-0'>
      Your Send to Kindle email is
    </label>
    <TextInput
      id='kindle-address-input'
      placeholder='bob@kindle.com'
      value={address}
      update={kindleAddress => updateUser({ kindleAddress })}
    ...
    <>
      <p className='mb-3 sm:mb-0'>
        ...
        <Link href='/dashboard/integration-help' shallow>
          <a>
            <HelpCircle className='inline-block ml-2 mb-1' height='0.95rem' />
          </a>
        </Link>
      </p>
      <Btn
        ...
        className='sm:w-60'
      >
        <RefreshCw height='1em' className='mr-3' />
        Generate an API Key
      </Btn>
