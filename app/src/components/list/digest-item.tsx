...
export const articleCountLabel = (n?: number): string =>
  !n ? 'No articles' : n === 1 ? '1 article' : `${n} articles`

export const DigestItem: FC<DigestItemProps> = ({ data }) => (
  <li className='list-none'>
    <Link href={`/digests/${data.id}`}>
      <Anchor
        aria-label={`View digest from ${humaniseDate(data.deliveredAt)}`}
        naked
        small
